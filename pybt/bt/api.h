...
//            storage.sync_schema(true);
    ...
    public:
        ...
        void save(const std::string &db_path, const std::string &table_name = "order") {
            ...
            // 分批次写入
            const int chunksize = 5000; // 每次插入的数据量
            int n = store_.size(); // 总的数据量
            for (int i = 0; i < n; i += chunksize) {
                int j = std::min(i + chunksize, n); // 下一批数据的结束位置
                auto begin = store_.begin() + i; // 当前批次数据的开始迭代器
                auto end = store_.begin() + j; // 当前批次数据的结束迭代器
                storage.replace_range(begin, end); // 使用 replace_range 方法插入数据
            }
        }
    };
